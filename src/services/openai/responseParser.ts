...
export class OpenAIResponseParser {
  static parseResponse(text: string, request: GenerationRequest, generationTimeMs: number): GenerationResult {
    ...
    console.log('Raw OpenAI response preview:', text.slice(0, 500));
    ...
    console.log('Successfully parsed JSON from OpenAI:', parsedData);
  ...
  private static extractJsonFromResponse(text: string): any {
    ...
    console.error('All JSON parsing strategies failed for OpenAI response');
    return null;
  }
